...
- Hardens Home Assistant media-user setup so failed Home Assistant only routing
  or display patch steps are reported as setup failures instead of
...
- Update the native C300X device agent from Home Assistant so the new forwarding
  mode, routing patch support, display labels and capture APIs are installed.
