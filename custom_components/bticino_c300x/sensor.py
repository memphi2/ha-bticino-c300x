...
_MEDIA_READINESS_STATUS_OPTIONS = ("ready", "warning", "blocked", "unavailable")
_MEDIA_READINESS_REQUIRED_SELF_TEST_CHECKS = (
    "capabilities",
    "firewall",
    "rtsp",
    "talkback_rtp",
...
async def async_setup_entry(
    ...
    entities: list[SensorEntity] = [
        ...
        C300XMediaReadinessSensor(entry),
    ]
...
class C300XMediaReadinessSensor(C300XConnectionDiagnosticSensor):
    """Summarize whether C300X media features are ready for normal use."""

    _attr_device_class = SensorDeviceClass.ENUM
    _attr_entity_category = EntityCategory.DIAGNOSTIC
    _attr_options = list(_MEDIA_READINESS_STATUS_OPTIONS)
        ...
        for signal in (
            ...
            self.async_on_remove(
                async_dispatcher_connect(
                    self.hass,
                    signal,
                    self._handle_readiness_changed,
                )
            )
    ...
    def native_value(self) -> str:
        ...
        return _media_readiness(self._entry)["status"]

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return non-sensitive readiness details."""

        readiness = _media_readiness(self._entry)
        return {
            key: value
...
def _agent_diagnostics_status(
    ...
    *,
    ...
    ui_event_waiters = diagnostics.get("ui_event_waiters")
    ui_event_capacity = diagnostics.get("ui_event_waiter_capacity")
    if (
        isinstance(ui_event_waiters, int)
        and isinstance(ui_event_capacity, int)
        and ui_event_capacity > 0
        and ui_event_waiters >= ui_event_capacity
    ):
        return "display_event_watchdog"
...
def _agent_diagnostics_status_reason(status: str) -> str:
    ...
    return {
        ...
        "display_event_watchdog": "display_ui_event_watchdog_triggered",
...
def _media_readiness(entry: ConfigEntry) -> dict[str, Any]:
    ...
    check_results = {
        name: _self_test_check_ok(checks, name)
        for name in _MEDIA_READINESS_REQUIRED_SELF_TEST_CHECKS
    }
    for name, ok in check_results.items():
        if ok is False:
            reason = _self_test_check_reason(checks, name)
            if _media_readiness_optional_ipv6_failure(name, reason, checks):
    ...
    callback_url_ok = _event_callback_ready(connection_state)
    ...
    return {
        ...
        "media_user_ok": _media_user_ready(
        ...
        "warnings": warnings,
        "recommended_action": _media_readiness_action(status, failed, warnings),
    }


def _self_test_check_ok(checks: Mapping[str, Any], name: str) -> bool | None:
...
def _self_test_check_reason(checks: Mapping[str, Any], name: str) -> str:
    check = checks.get(name)
    if not isinstance(check, Mapping):
        return ""
...
def _media_readiness_optional_ipv6_failure(
    check_name: str,
    reason: str,
...
def _media_user_ready(
    device_user_status: Mapping[str, Any],
    checks: Mapping[str, Any],
) -> bool | None:
    user_check = _self_test_check_ok(checks, "homeassistant_user")
    routing_check = _self_test_check_ok(checks, "device_routing")
...
def _event_callback_ready(connection_state: Any) -> bool | None:
