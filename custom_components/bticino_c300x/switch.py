...
async def async_setup_entry(
    ...
    if entry_video_enabled(entry) and supports_capability(entry, "device_user"):
        entities.append(C300XHomeAssistantMediaUserSetupSwitch(entry))
...
class C300XHomeAssistantMediaUserSetupSwitch(C300XEntity, SwitchEntity):
    """Apply or restore the full Home Assistant media-user setup."""
    ...
    def is_on(self) -> bool | None:
        """Return whether the complete Home Assistant media user is active."""
        ...
        return (
            ...
            and self._status.get("device_routing_applied") is True
            and self._status.get("media_user_label_applied") is True
        )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        try:
            status = await self._entry.runtime_data.api.async_restore_homeassistant_media_user_setup()
        except C300XAgentApiUnsupportedError as err:
            raise HomeAssistantError(
                "The installed C300X device agent does not support device-user restore"
            ) from err
        except C300XAgentApiError as err:
            raise HomeAssistantError("C300X Home Assistant media user restore failed") from err
        await self._store_status(status)

    async def async_update(self) -> None:
        """Refresh setup state through the agent read-only endpoint."""
...
class C300XGuiFunctionPatchSwitch(C300XEntity, SwitchEntity):
    ...
    @property
    def is_on(self) -> bool | None:
        """Return whether the Display patch is fully applied."""
    ...
    @property
    def available(self) -> bool:
        """Return true when all Display patch maintenance actions are advertised."""
    ...
    async def async_update(self) -> None:
        """Refresh Display patch state through the read-only status endpoint."""
    ...
    async def async_added_to_hass(self) -> None:
        """Subscribe to Display patch state updates."""
