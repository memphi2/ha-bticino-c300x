...
Page {
    ...
    function isArmedState() {
        return alarmRawState === "armed_home"
            || alarmRawState === "armed_away"
            || alarmRawState === "armed_night"
            || alarmRawState === "armed_custom_bypass"
            || alarmRawState === "armed_vacation"
    }
    ...
    function feedbackTitle() {
        ...
        if (isArmedState()) return commandLabel(Api.commandForState(alarmRawState))
        return stateLabel()
    }
    ...
    function feedbackDetail() {
        ...
        var pin = pinCode.length > 0 ? uiText("pin_label") + ": " + pinMask() : ""
        ...
        if (alarmRawState === "disarmed") {
            var sensors = Api.alarmOpenSensorsText(page)
            if (sensors.length > 0) return sensors
            if (!selectedCommandReady && selectedCommandStatus.length > 0) return selectedCommandStatus
            return uiText("sensor_ok")
        }
        if (commandRequiresPin(selectedCommand)) {
            if (timer.length > 0) return timer + "  " + commandLabel(selectedCommand)
            return uiText("pin_label") + ":"
        }
        if (timer.length > 0) return timer + "  " + commandLabel(selectedCommand)
