...
Device setup actions are explicit. They are not applied by normal Home
Assistant startup. Display setup changes only the C300X display files needed for
display pages. Media-user setup prepares the local C300X account and routing
files used by Home Assistant media features. Setup actions write only changed
files, keep one original backup, and restore the root filesystem to read-only
...
- Home Assistant media-user setup, when you do not use Home Assistant media
  features.
...
- **Ring Call** answers the real incoming doorbell call reported by the agent
  when the C300X forwarding mode routes the call to Home Assistant.
...
The **Forwarding** select has three visible modes:

- **Smartphone**: the original all-smartphones forwarding path is active. Home
...
The **Home Assistant** mode requires the native agent update and the Home
Assistant media-user setup. The C300X display label is localized in English,
...
- one Home Call card for the audio-only local Home Call.
...
visible state is **Home Assistant**.
