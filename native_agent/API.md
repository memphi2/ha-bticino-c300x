...
Side effects: none. The endpoint must not write files, patch QML, patch
...
```json
{
  ...
  "checks": {
    ...
    "inhouse_patch": {"ok": true, "reason": "inhouse_patch_ok"},
...
- `inhouse_patch`: reads the binary patch hash and QML patch status. It does
  not apply or restore patches.
