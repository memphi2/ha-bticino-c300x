...
installs that core hook plus the complete Display patch: MainApp navigation,
...
removes only the optional Display patch and keeps the core media hook;
`restore-all` is reserved for agent removal and restores the core hook too. The
script backs up original display files under
...
file differs. The `status` action is read-only and reports both the optional
Display patch state and the core media-hook state.
...
readiness, Home Assistant media-user routing, device routing status, and startup
