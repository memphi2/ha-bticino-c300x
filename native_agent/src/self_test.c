...
static struct self_test_qml_status qml_status(const struct c300x_config *config)
{
    ...
    c300x_copy_string(status.inhouse_state, sizeof(status.inhouse_state), "unavailable");
    ...
    if (!run_qml_status_script(config->maintenance_qml_patch_script, output, sizeof(output))) {
        ...
        c300x_copy_string(status.inhouse_state, sizeof(status.inhouse_state), "script_failed");
        return status;
    }
    (void)json_bool_field(output, "ok", &ok);
    status.ok = ok;
    (void)json_bool_field(output, "inhouse_patched", &status.inhouse_patched);
    json_string_field(output, "state", status.state, sizeof(status.state));
    json_string_field(output, "inhouse_state", status.inhouse_state, sizeof(status.inhouse_state));
    if (status.state[0] == '\0') {
        c300x_copy_string(status.state, sizeof(status.state), "unknown");
    }
    if (status.inhouse_state[0] == '\0') {
        c300x_copy_string(status.inhouse_state, sizeof(status.inhouse_state), "unknown");
    }
    return status;
}
...
)
{
    ...
    char binary_state_json[96];
    char binary_error_json[C300X_INHOUSE_PATCH_ERROR_LEN * 6 + 3];
    char qml_state_json[96];
    char qml_inhouse_state_json[96];
    ...
    const char *inhouse_reason = "not_required_without_homeassistant_user";
    ...
    if (video_enabled) {
        ...
        binary_read_ok = c300x_inhouse_binary_patch_read_status(&binary_status);
        if (user_status.homeassistant_user_present) {
            inhouse_ok = binary_read_ok
                && binary_status.patched
                && qml.available
                && qml.ok
                && qml.inhouse_patched;
            if (!binary_read_ok) {
                inhouse_reason = "inhouse_binary_status_failed";
            } else if (!binary_status.patched) {
                inhouse_reason = "inhouse_binary_patch_missing";
            } else if (!qml.available) {
                inhouse_reason = "inhouse_qml_status_unavailable";
            } else if (!qml.ok) {
                inhouse_reason = "inhouse_qml_status_failed";
            } else if (!qml.inhouse_patched) {
                inhouse_reason = "inhouse_qml_patch_missing";
            } else {
                inhouse_reason = "inhouse_patch_ok";
            }
        }
    }
    ...
    json_string(qml.inhouse_state, qml_inhouse_state_json, sizeof(qml_inhouse_state_json));
    ...
    return c300x_appendf(
        ...
        "\"inhouse_patch\":{\"ok\":%s,\"reason\":\"%s\",\"binary_supported\":%s,\"binary_patched\":%s,"
        "\"binary_state\":%s,\"binary_error\":%s,\"qml_available\":%s,\"qml_ok\":%s,"
        "\"qml_state\":%s,\"qml_inhouse_state\":%s,\"qml_inhouse_patched\":%s},"
        ...
        firmware_json,
        bool_json(capabilities_ok && firewall_ok && rtsp_ok && talkback_ok && user_ok && inhouse_ok && startup_ok),
        ...
        user_error_json,
        bool_json(inhouse_ok),
        inhouse_reason,
        bool_json(binary_status.supported),
        bool_json(binary_status.patched),
        binary_state_json,
        binary_error_json,
        bool_json(qml.available),
        bool_json(qml.ok),
        qml_state_json,
        qml_inhouse_state_json,
        bool_json(qml.inhouse_patched),
