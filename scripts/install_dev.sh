...
Options:
  ...
  --no-qml-apply   Upload QML sources but do not apply/reload the GUI patch.
  --agent          Build and upload the native ARMHF agent binary too.

Defaults:
  - HA install uses the GVFS SMB config share.
  - Device install uploads project-owned QML/JS and qml_patch.sh.
  - The active C300X GUI patch is applied once.
