  ...
  const page = {{
    ...
    feedbackTitle() {{
      ...
      feedbackDetail() {{
        ...
        const pin = this.pinCode.length > 0 ? this.uiText("pin_required") + " " + this.pinMask() : "";
...
function assertBlockedModesLookLikeAlarmoCard() {{
  ...
  assert.strictEqual(page.modeReadyColor("arm_away"), "#ff6b6b");
    ...
    assert.strictEqual(page.feedbackDetail(), "Abwesend");
...
function assertDisarmRequiresConfiguredPin() {{
  ...
  page.appendDigit("2");
    page.appendDigit("3");
    page.appendDigit("4");
    assert.strictEqual(page.feedbackDetail(), "Aus  PIN benoetigt ****");
...
function assertDisarmPinFeedbackWinsDuringDelay() {{
  const page = reset(statePayload("pending", [
    ...
    page.appendDigit("1");
    page.appendDigit("2");
    assert.strictEqual(page.feedbackDetail(), "25 s  Aus  PIN benoetigt **");
}}
